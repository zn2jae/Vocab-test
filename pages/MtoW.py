...
if 'words_to_test' not in st.session_state:
    st.session_state.words_to_test = []
...
def start_quiz_mtow():
    if st.session_state.quiz_day:
        ...
        if df is not None and not df.empty:
            ...
            st.session_state.current_word_index = 0
            st.session_state.results = []
            st.session_state.words_to_test = list(st.session_state.quiz_data['word'])
        else:
...
if not st.session_state.quiz_started:
    ...
    if st.session_state.current_word_index < st.session_state.total_questions:
        ...
        # 뜻을 세미콜론(;) 기준으로 분리하여 첫 번째 뜻만 표시 (UI를 더 깔끔하게)
        display_meaning = meaning.split(';')[0].strip()
        
        # 쉼표(,)도 분리하여 첫 번째 뜻만 표시
        if ',' in display_meaning:
            display_meaning = display_meaning.split(',')[0].strip()
        
        st.markdown(f"### **뜻:** `{display_meaning}`")
        
        user_answer = st.text_input("단어를 입력하세요:", key=f"word_input_{st.session_state.current_word_index}")
